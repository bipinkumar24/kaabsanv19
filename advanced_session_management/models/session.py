...
class user_login(Home):
    ...
    @http.route(['/web', '/odoo', '/odoo/<path:subpath>', '/scoped_app/<path:subpath>'], type='http', auth="none", readonly=False)
    def web_client(self, s_action=None, **kw):
        response = super(user_login, self).web_client(s_action, **kw)
        login_log = request.env['login.log'].sudo().search([('user_id','=',request.env.uid),('session_id','=',False)], order='id desc',limit=1)
        if login_log:
