        ...
        try:
            ...
            if self._name not in exclude_models.ignore_model and moduel_state == 'installed':
                ...
                for record in self:
                    ...
                    login_log = record.env['res.device.log'].sudo().search([('session_identifier','=',request.session.sid[:42])],limit=1)
        ...
        try:
            ...
            for record in res:
                if record._name not in exclude_models.ignore_model and moduel_state == 'installed':
                    ...
                    login_log = self.env['res.device.log'].sudo().search([('session_identifier','=',request.session.sid[:42])],limit=1)
                    if login_log and not self._transient:
                        disable = str(config_parameter_obj.get_param('disable_log'))
                        if disable == 'False':
