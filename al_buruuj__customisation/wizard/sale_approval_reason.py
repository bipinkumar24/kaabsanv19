...
class SaleApprovalReason(models.TransientModel):
    ...
    ], 'Approve Type', copy=False, required=True)
    requested_discount = fields.Float('Requested Discount')
    notes = fields.Text('Notes')

    def approve(self):
        self.ensure_one()
        sale_br_obj = self.env['sale.order'].browse(self.env.context.get('active_id'))
        ...
        if self.approval_for == 'for_amount':
            ...
            if user_search:
                ...
                raise UserError(_('Approver is not set for this amount limit. Please allocate approver.'))
        ...
        if self.approval_for == 'for_discount':
            if self.requested_discount:
                user_search_discount = user_obj.search([('sale_order_discount_limit', '>=', self.requested_discount), ('sale_order_can_approve', '=', 'yes')], order='sale_order_discount_limit')
                if user_search_discount:
                    discount_approver_user_id = user_search_discount[0]
                else:
                    raise UserError(_('Approver is not set for this discount limit. Please allocate approver.'))
