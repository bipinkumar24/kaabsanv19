...
class Account(models.Model):
    ...
    def _get_default_department(self):
        user = self.env.user
        return user.employee_id.department_id.id if user.employee_id.department_id else False
    ...
    def search(self, args, **kwargs):
        ...
        if self.env.user.has_group('expense_requests.expensess_manager'):
            ...
            args += ['|', ('user_id.employee_id.department_id', '=', department.id),
                     ('user_id.employee_id.department_id', 'child_of', department.id)]
    ...
    def _get_next_approval_id(self):
        if not sql.table_exists(self.env.cr, 'approval_level_account'):
            return False
    ...
    is_button = fields.Boolean('Can Approve', compute='_compute_is_button')
    is_first = fields.Boolean('Is First Approval Level', compute='_compute_is_first')
    is_last_level = fields.Boolean('Is Last Approval Level', compute='_compute_is_button')
    ...
    require_custom_gm = fields.Boolean(string='Requires Custom GM Approval')
