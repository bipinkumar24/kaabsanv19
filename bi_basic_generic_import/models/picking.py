...
class import_pickingss(models.TransientModel):
    ...
    location_id = fields.Many2one(
        ...
        default=lambda self: self.env['stock.picking.type'].browse(self.env.context.get('default_picking_type_id')).default_location_src_id,
        required=True,
        )
    location_dest_id = fields.Many2one(
        'stock.location', "Destination Location Zone",
        default=lambda self: self.env['stock.picking.type'].browse(self.env.context.get('default_picking_type_id')).default_location_dest_id,
        required=True,
        )
