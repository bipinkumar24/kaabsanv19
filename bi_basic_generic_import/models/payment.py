...
class gen_salereceipt(models.TransientModel):
    ...
    def _create_customer_payment(self, val):
        ...
        pay_id = self.find_payment_method(payment_journal, val['payment_option'])
    ...
    def find_payment_method(self, journal_id, payment_option):
        journal = self.env['account.journal'].browse(journal_id)
        if payment_option == 'supplier':
        ...
        if not payment_method_line:
            raise ValidationError(_('Manual payment method is not configured on journal "%s".') % journal.display_name)
        return payment_method_line.id
