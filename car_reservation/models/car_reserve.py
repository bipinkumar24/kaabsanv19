...
def reservation_geo_find(addr, apikey=False):
    ...
    if result["status"] != "OK":
        if result.get("error_message"):
            ...
            error_msg = _(
                "Unable to geo-locate, received the error:\n%s"
...
def reservation_geo_query_address(
...
class CarReservation(models.Model):
    ...
    name = fields.Char("Name", help="The name of the reservation")
    ...
    approvar_id = fields.Many2one("res.users", "Approver")
    ...
    end_date = fields.Datetime(string="End Time", help="Ending time of reservation")
    ...
    drop_latitude = fields.Float(
        related="drop_location_id.partner_latitude", string="Drop Location Latitude"
    )
    drop_longitude = fields.Float(
        related="drop_location_id.partner_longitude", string="Drop Location longitude"
    )
    notes = fields.Text(string="Details")
    value = fields.Float(string="Kilometer (Distance)")
    reservation_no = fields.Char(string="Reservation Number", copy=False)
    pick_street = fields.Char(
        related="pickup_location_id.street", string="Pick Location Street"
    )
    pick_street2 = fields.Char(
        related="pickup_location_id.street2", string="Pick Location Street2"
    )
    pick_zip = fields.Char(related="pickup_location_id.zip", string="Pick Location Zip")
    pick_city = fields.Char(
        related="pickup_location_id.city", string="Pick Location City"
    )
    pick_state_id = fields.Many2one(
        related="pickup_location_id.state_id",
        string="Pick Location State",
        ondelete="restrict",
    )
    pick_country_id = fields.Many2one(
        related="pickup_location_id.country_id",
        string="Pick Location Country",
        ondelete="restrict",
    )
    pick_website = fields.Char(
        related="pickup_location_id.website",
        string="Pick Location Website",
        help="Website",
    )
    pick_phone = fields.Char(
        related="pickup_location_id.phone", string="Pick Location Phone"
    )
    drop_street = fields.Char(
        related="drop_location_id.street", string="Drop Location Street"
    )
    drop_street2 = fields.Char(
        related="drop_location_id.street2", string="Drop Location Street2"
    )
    drop_zip = fields.Char(related="drop_location_id.zip", string="Drop Location Zip")
    drop_city = fields.Char(
        related="drop_location_id.city", string="Drop Location City"
    )
    drop_state_id = fields.Many2one(
        related="drop_location_id.state_id",
        string="Drop Location State",
        ondelete="restrict",
    )
    drop_country_id = fields.Many2one(
        related="drop_location_id.country_id",
        string="Drop Location Country",
        ondelete="restrict",
    )
    drop_website = fields.Char(
        related="drop_location_id.website",
        string="Drop Location Website",
        help="Website",
    )
    drop_phone = fields.Char(
        related="drop_location_id.phone", string="Drop Location Phone"
    )
    ...
    def check_negative_value(self):
        ...
        for rec in self:
            if rec.value < 0.00:
                raise UserError(_("Odometer Value Must be in Positive value !!"))
    ...
    def check_dates(self):
        ...
        for rec in self:
            if rec.start_date and rec.end_date and rec.start_date >= rec.end_date:
                raise UserError(_("Start Time Should be less than the End Time!"))
    ...
    def get_emp_domain(self, emp_type):
        ...
        res = {
            "condition1": [
                ...
                ("employee_id", "=", rev.employee_id and rev.employee_id.id or False),
            ],
            "condition2": [
                ("id", "!=", rev.id),
                ("vehicle_id", "=", rev.vehicle_id.id),
            ...
            "condition5": [
                ...
                ("driver_id", "=", rev.driver_id.id),
                ("state", "in", ["approved", "reserved"]),
                ("partner_id", "=", rev.partner_id and rev.partner_id.id or False),
            ],
            "condition6": [
                ("id", "!=", rev.id),
                ("state", "in", ["approved", "reserved"]),
    ...
    def check_reservation_validation(self):
        ...
        for rev in self:
            if rev.start_date and rev.end_date and rev.driver_id and rev.vehicle_id:
    ...
    def write(self, vals):
        """Override write method to check reservation constraints."""
        res = super().write(vals)
        for reservation in self:
            if (
                vals.get("vehicle_id", False)
                or vals.get("driver_id", False)
                or vals.get("partner_id", False)
                or vals.get("employee_id", False)
                or vals.get("state", False)
            ):
                reserv = reservation.check_reservation_validation()
                if reserv:
                    raise UserError(
    ...
    def update_state(self):
        ...
        template = self.env.ref("car_reservation.custom_car_reservation_template")
        for rec in self:
            state_to_update = context.get("state", False)
            if (
                state_to_update == "approved"
                and self.env.uid != SUPERUSER_ID
                and self.env.uid != rec.approvar_id.id
            ):
                raise UserError(
                    _("Only Reservation Approver can approve this Reservation!")
                )
                ...
                data = {
                    ...
                    "pickup_location_id": rec.pickup_location_id
                    and rec.pickup_location_id.id
                    or False,
                    "drop_location_id": rec.drop_location_id
                    and rec.drop_location_id.id
                    or False,
            ...
            if state_to_update == "approved" and not rec.reservation_no:
                rec.reservation_no = (
                    self.env["ir.sequence"].next_by_code("car.reservation") or ""
                )
            if state_to_update:
    ...
    @api.onchange("pickup_location_id", "drop_location_id")
    def change_locations(self):
        for location in self:
            if location.pickup_location_id and location.drop_location_id:
                pick_loc = (location.latitude, location.longitude)
                drop_loc = (location.drop_latitude, location.drop_longitude)
                location.value = geodesic(pick_loc, drop_loc).km
...
class FleetVehicle(models.Model):
    ...
    reservation_count = fields.Integer(
        compute="_compute_count_vehicle_reservations", string="Reservations"
    )
    state = fields.Selection(
        [
            ("inspection", "Draft"),
    ...
    def _compute_get_odometer(self):
        """Compute total odometer from all records for this vehicle."""
        odometer_obj = self.env["fleet.vehicle.odometer"]
        for record in self:
            vehicle_odometers = odometer_obj.search(
                [("vehicle_id", "=", record.id)], order="value desc"
            )
            record.odometer = sum(odom.value for odom in vehicle_odometers) or 0.0
...
class FleetVehicleOdometer(models.Model):
    ...
    pickup_location_id = fields.Many2one(
        "reservation.location", ondelete="restrict", string="Pick-up Location"
    )
    drop_location_id = fields.Many2one(
        "reservation.location", ondelete="restrict", string="Drop Location"
    )
