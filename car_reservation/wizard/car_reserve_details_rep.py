...
from odoo.tools import DEFAULT_SERVER_DATE_FORMAT as DF, date_utils
...
class WizCarReserveDetailsReport(models.TransientModel):
    ...
    date_from = fields.Datetime(
        string="Date From",
        default=lambda self: date_utils.start_of(fields.Datetime.now(), "month"),
    )
    date_to = fields.Datetime(
        string="Date To",
        default=lambda self: date_utils.end_of(fields.Datetime.now(), "month"),
    )
    ...
    def _constraint_date_limit(self):
        ...
        if self.date_from and self.date_to:
            if self.date_from > self.date_to:
                raise UserError(_("End date must be greater " "than the start date !"))
        return True
    ...
    def print_report(self):
        ...
        dom = [("start_date", ">=", self.date_from), ("end_date", "<=", self.date_to)]
        if self.vehicle_ids:
        ...
        if not reservation_recs:
            raise UserError(
                _("No Car Reservation records found between " "this duration! ")
            )
        for vehicle_rec in vehicle_recs:
            ...
            sheet_name = (
                vehicle_model and vehicle_model.name + "-" + vehicle_rec.license_plate
            )
            ...
            utc_timestamp = pytz.utc.localize(min_hr_dt.get("mn"), is_dst=False)
            ...
            for vehice_reserve_rec in vehicle_reserv_recs:
                ...
                disp_veh_res_st_dt = datetime.strftime(veh_res_st_dt_str_frmt, df)
                disp_veh_res_en_dt = datetime.strftime(veh_res_en_dt_str_frmt, df)
                ...
                # Fetching Times.
                veh_res_st_time = datetime.strftime(veh_res_st_dt_str_frmt, "%H")
                veh_res_en_time = datetime.strftime(veh_res_en_dt_str_frmt, "%H")
                # fetching driver name first from reservation record
                # if its not there fetching from reservation's vehicle's
                # driver record.
                driver_name = (
                    vehice_reserve_rec.driver_id
                    and vehice_reserve_rec.driver_id.name
                    or vehice_reserve_rec.vehicle_id
                    and vehice_reserve_rec.vehicle_id.driver_id
                    and vehice_reserve_rec.vehicle_id.driver_id.name
                    or "-"
                )
                # If both date and time are same means need to write in
                # single row.pickup
                if (
                    disp_veh_res_st_dt == disp_veh_res_en_dt
                    and veh_res_st_time == veh_res_en_time
                ):
                    col_no1 = vehicle_time_dict[veh_res_st_time]
                    worksheet.write(
                        row_no1[0], col_no1[0], driver_name or "-", merge_cell_format
                    )
                else:
                    col_no1 = vehicle_time_dict[veh_res_st_time]
        ...
        form_view = self.env.ref("car_reservation.wiz_reserve_details_download_wiz")
...
class WizDownloadReserveRep(models.TransientModel):
    ...
    file = fields.Binary("Click On Download Link To Download Xlsx File", readonly=True)
