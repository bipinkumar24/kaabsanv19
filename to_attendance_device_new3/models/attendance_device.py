...
class AttendanceDevice(models.Model):
    ...
    def _get_default_attendance_states(self):
        ...
        attendance_device_state_code_0 = self.env.ref('to_attendance_device_new3.attendance_device_state_code_0')
        if attendance_device_state_code_0:
            att_state_ids += attendance_device_state_code_0
        attendance_device_state_code_1 = self.env.ref('to_attendance_device_new3.attendance_device_state_code_1')
        if attendance_device_state_code_1:
            att_state_ids += attendance_device_state_code_1

        # Overtime Working Attendance
        attendance_device_state_code_4 = self.env.ref('to_attendance_device_new3.attendance_device_state_code_4')
        if attendance_device_state_code_4:
            att_state_ids += attendance_device_state_code_4
        attendance_device_state_code_5 = self.env.ref('to_attendance_device_new3.attendance_device_state_code_5')
    ...
    name = fields.Char(string="Name", required=True, help='The name of the attendance device', tracking=True,
    ...
    ip = fields.Char(string="IP / Domain Name", required=True, tracking=True, copy=False, readonly=True,
                     states={'draft': [('readonly', False)]},
                     help='The accessible IP or Domain Name of the device to get device\'s attendance data',
                     default='0.0.0.0')
    port = fields.Integer(string="Port", required=True, default=4370, tracking=True, readonly=True,
                          states={'draft': [('readonly', False)]})
    ...
    active = fields.Boolean(string='Active', default=True, tracking=True, readonly=True,
                            states={'draft': [('readonly', False)]})
    ...
    def connect(self):
        ...
        def post_message():
            email_template_id = self.env.ref('to_attendance_device_new3.email_template_attendance_device')
    ...
    def getFirmwareVersion(self):
        ...
        try:
            ...
            return zk.get_firmware_version()
        except Exception as e:
            _logger.error(e)
    ...
    def setUser(self, uid=None, name='', privilege=0, password='', group_id='', user_id='', card=0):
        try:
            ...
            self.enableDevice()
            self.disableDevice()
            return zk.set_user(uid, name, privilege, password, group_id, user_id, card)
        except Exception as e:
            _logger.info(e)
    ...
    def delUser(self, uid, user_id):
        ...
        try:
            ...
            self.enableDevice()
            self.disableDevice()
            return zk.delete_user(uid, user_id)
        except Exception as e:
            _logger.error(e)
    ...
    def upload_finger_templates(self, uid, name, privilege, password, group_id, user_id, fingers):
        ...
        try:
            ...
            self.enableDevice()
            self.disableDevice()
            return zk.save_user_template(user, fingers)
        except Exception as e:
            _logger.error(e)
    ...
    def delFingerTemplate(self, uid, fid, user_id):
        ...
        try:
            ...
            self.enableDevice()
            self.disableDevice()
            return zk.delete_user_template(uid, fid, user_id)
        except Exception as e:
            _logger.error(e)
    ...
    def getAttendance(self):
        ...
        try:
            ...
            if post_err_msg and self.download_error_notification:
                email_template_id = self.env.ref('to_attendance_device_new3.email_template_error_get_attendance')
                # self.post_message(email_template_id)
            self.enableDevice()
            self.disconnect()
    ...
    def _download_users_by_user_id(self):
        ...
        for r in self:
            ...
            _logger.info('eeeeeuidddddgudusgduysgduygsuydsuygd')
            ...
            _logger.info(existing_user_ids)
            _logger.info('eeeeeuiddddwwwwwwdgudusgduysgduygsuydsuygd444')
            for device_user in device_users:
    ...
    def action_attendance_download(self):
        DeviceUserAttendance = self.env['user.attendance'].sudo()
        AttendanceUser = self.env['attendance.device.user'].sudo()
        # DeviceUserAttendance.get_new_date()
        ...
        for r in self:
            ...
            for attendance in attendance_data:
                ...
                if user_id:
                    ...
                    device_users = self.env['attendance.wizard'].sudo().search([('time_date', '!=', False)])
                    for i in device_users:
                        time_date = i.time_date
                    time_date = datetime.strftime(time_date, '%Y-%m-%d %H:%M:%S')
                    if not duplicate_attend and str_utc_timestamp > time_date:
                        ...
                        str_utc_timestamp_add = datetime.strptime(str_utc_timestamp, '%Y-%m-%d %H:%M:%S')
                        date_condition = date.strftime(attendance.timestamp, '%Y-%m-%d')
                        users = self.env['user.attendance'].sudo().search([('user_id', '=', user_id.id),
                                                                    ('timestamp', '>=', time_date)])
                        cont = 0
                        reduce_date = 1
                        if users:
                        ...
                        for user in users:
                            ...
                            if len(users) % 2 != 0:
                                if len(users) >= 2:
                                    str_utc_timestamp_add = datetime.strptime(str_utc_timestamp, '%Y-%m-%d %H:%M:%S')
                                    timestamp = str_utc_timestamp_add - users[0].timestamp
                                    count_18 = timestamp.total_seconds() / 3600
                                    diff = relativedelta.relativedelta(str_utc_timestamp_add
                                                                       , users[0].timestamp)
                                    diff_in_months = diff.months + diff.years * 12
                                    if count_18 < 0:
                                        reduce_date = 8
                                    # elif count_18 < 13.0:
                                    #     str_utc_timestamp_add = datetime.strptime(str_utc_timestamp,
                                    #                                               '%Y-%m-%d %H:%M:%S')
                                    #     timestamp = str_utc_timestamp_add - users[1].timestamp
                                    #     count_18 = timestamp.total_seconds() / 3600
                                    #     if count_18 < 13.0:
                                    #         users[0].unlink()
                                    elif count_18 > 0 and str_utc_timestamp_add.date() == users[1].date == users[
                                        0].date:
                                        users[0].valid = True
                                        break
                            ...
                            if len(users) >= 2 and len(users) % 2 == 0:
                                ...
                                timestamp = str_utc_timestamp_add - users[1].timestamp
                                count_18 = timestamp.total_seconds() / 3600
                                if users[0].date == date_condition and count_18 > 13:
                                    reduce_date = 8
                                if count_18 < 0:
                                    reduce_date = 8

                                elif cont < user_len and count_18 < 13.0 and count_18 > 0:
                                    # user.sudo().unlink()
                                    break
                                else:
                                    break
                        if reduce_date == 1:
                            id = DeviceUserAttendance.sudo().create(vals)
    ...
    def action_attendance_download_auto(self):
        DeviceUserAttendance = self.env['user.attendance'].sudo()
        AttendanceUser = self.env['attendance.device.user'].sudo()
        # DeviceUserAttendance.get_new_date()
        ...
        for r in self:
            ...
            for attendance in attendance_data:
                ...
                if user_id:
                    ...
                    time_date = datetime.today() - timedelta(days=1)
                    time_date = datetime.strftime(time_date, '%Y-%m-%d %H:%M:%S')
                    if not duplicate_attend and str_utc_timestamp > time_date:
            ...
            for template in template_data:
                ...
                if (str(template.uid) + '_' + str(template.fid)) not in existing_finger_template_ids:
                    ...
                    if existing:
                        existing.write(vals)
            if vals_list:
                FingerTemplate.create(vals_list)
        return
        ...
        for r in self:
            ...
            if attendance_clear_safe:
                ...
                if r.auto_clear_attendance_error_notif:
                    email_template_id = self.env.ref('to_attendance_device_new3.email_template_not_safe_to_clear_attendance')
    ...
    def action_device_information(self):
        ...
        for r in self:
            try:
                ...
                r.serialnumber = zk.get_serialnumber()
                r.platform = zk.get_platform()
                r.oem_vendor = zk.get_oem_vendor()
                r.fingerprint_algorithm = zk.get_fp_version()
                r.device_name = zk.get_device_name()
                r.work_code = zk.get_workcode()
            except Exception as e:
                _logger.error(e)
            finally:
                cr.commit()
    ...
    def action_view_users(self):
        action = self.env.ref('to_attendance_device_new3.device_user_list_action')
        ...
        if self.device_users_count != 1:
            ...
            res = self.env.ref('to_attendance_device_new3.attendance_device_user_form_view', False)
    ...
    def action_view_attendance_data(self):
        self.ensure_one()
        action = self.env.ref('to_attendance_device_new3.action_user_attendance_data')
        result = action.read()[0]
        ...
        if total_att_records != 1:
            ...
            res = self.env.ref('to_attendance_device_new3.view_attendance_data_form', False)
            result['views'] = [(res and res.id or False, 'form')]
    ...
    def action_view_mapped_employees(self):
        ...
        if self.mapped_employees_count != 1:
            ...
            res = self.env.ref('to_attendance_device_new3.view_employee_form', False)
            result['views'] = [(res and res.id or False, 'form')]
    ...
    def action_view_finger_template(self):
        self.ensure_one()
        action = self.env.ref('to_attendance_device_new3.action_finger_template')
        result = action.read()[0]
        ...
        if total_finger_template_records != 1:
            ...
            res = self.env.ref('to_attendance_device_new3.view_finger_template_form', False)
            result['views'] = [(res and res.id or False, 'form')]
