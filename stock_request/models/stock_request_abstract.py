...
class StockRequest(models.AbstractModel):
    ...
    @api.depends("product_id", "product_id.product_tmpl_id.uom_id", "product_id.product_tmpl_id.uom_ids")
    def _compute_allowed_uom_ids(self):
        for rec in self:
            if rec.product_id:
                rec.allowed_uom_ids = rec.product_id.product_tmpl_id.uom_id | rec.product_id.product_tmpl_id.uom_ids
            else:
                rec.allowed_uom_ids = self.env["uom.uom"]
    ...
    route_id = fields.Many2one(
        # Odoo 15 legacy model: "stock.location.route"
        "stock.route",
        string="Route",
    ...
    def _compute_route_ids(self):
        # Odoo 15 legacy model: self.env["stock.location.route"]
    ...
    def _check_product_uom(self):
        ...
        if any(
            ...
            and request.product_uom_id
            and not request.product_id.uom_id._has_common_reference(request.product_uom_id)
            for request in self
        ):
            raise ValidationError(
                _(
                    "You have to select a product unit of measure in the "
                    "same UoM family as the default unit "
