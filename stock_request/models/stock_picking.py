...
class StockPicking(models.Model):
    ...
    receipt_date = fields.Datetime(
        string='Purchase Receipt Date',
        compute='_compute_purchase_receipt_date',
        readonly=True,
    )

    @api.depends('purchase_id.date_planned')
    def _compute_purchase_receipt_date(self):
        for picking in self:
            picking.receipt_date = picking.purchase_id.date_planned
