...
class StockRequest(models.Model):
    ...
    def _compute_next_approval_user_id(self):
        for rec in self:
            rec.next_approval_user_ids = rec.next_approval_id.approval_user_ids
    ...
    # Odoo 15 legacy: states={"draft": [("readonly", False)]}
    name = fields.Char()
    ...
    # Odoo 15 legacy on the overrides below: states={"draft": [("readonly", False)]}
    warehouse_id = fields.Many2one(readonly=True)
    location_id = fields.Many2one(readonly=True)
    ...
    # Odoo 15 legacy override:
    # procurement_group_id = fields.Many2one(
    #     states={"draft": [("readonly", False)]}, readonly=True
    # )
    procurement_group_id = fields.Integer(
        "Procurement Group",
        readonly=True,
        help="Legacy Odoo 15 procurement group id. Odoo 19 uses stock.rule "
        "directly and no longer provides procurement.group.",
    )
    company_id = fields.Many2one(readonly=True)
    route_id = fields.Many2one(readonly=True)
    is_dyanamic = fields.Boolean(string="Is Dynamic")
    next_approval_id = fields.Many2one('approval.stock.request', string='Next Approval', tracking=True, copy=False)
    next_approval_user_ids = fields.Many2many(
        'res.users',
        string='Next Approval By',
        compute='_compute_next_approval_user_id',
    )
    ...
    @api.onchange('is_dyanamic')
    def dynamic_approval_data(self):
        if self.is_dyanamic:
            next_approval_id = self.env['approval.stock.request'].search([('level', '=', 1)], limit=1)
            self.next_approval_id = next_approval_id.id
        else:
    ...
    def action_approve(self):
        view_id = self.env.ref('stock_request.sr_remark_wizard_view_form').id
        return {'type': 'ir.actions.act_window',
                'name': _('Remarks'),
                'res_model': 'sr.remark.wizard',
                'target': 'new',
                'view_mode': 'form',
                'views': [[view_id, 'form']],
                }
    ...
    def _prepare_procurement_values(self, group_id=False):
        ...
        return {
            ...
            # Odoo 15 used: "group_id": group_id or self.procurement_group_id.id or False
            # Odoo 19 removed procurement.group, so stock.rule receives no group_id.
            ...
            try:
                ...
                procurements.append(
                    self.env["stock.rule"].Procurement(
                        ...
                        request.location_id,
                        request.name,
                        request.order_id.name or request.name,
                        self.env.company,
                        values,
                    )
                )
