...
class ImexInventoryReportWizard(models.TransientModel):
    ...
    def button_update_onhand_qty(self):
        """Compute move-based balance and write it to stock.quant."""
        report_obj = self.env["imex.inventory.report"]
        report = self.create(self._prepare_imex_inventory_report())
        report_obj.init_results(report)

        records = report_obj.search([])
        updated = 0
        for rec in records:
        ...
        return {
            ...
            'params': {
                ...
                'message': f'{updated} product(s) updated from move-based balance.',
                'type': 'success',
                'sticky': False,
            },
        }

    def button_view_details_pdf(self):
        vals = {}
        report = self.create(self._prepare_imex_inventory_report())
        init = self.env["imex.inventory.details.report"].init_results(report)
        details = self.env["imex.inventory.details.report"].search([])
        data = {
            ...
            'product_category_ids': self.product_category_ids.ids or False,
            'location_id': self.location_id.name if self.location_id else 'All',
            'name': self.product_ids[0].name
        }
        return self.env.ref('imex_inventory_report.action_pdf_imex').report_action(self, data=data)
