...
class LeaveCarryOver(models.Model):
    ...
    def _compute_allocations_count(self):
        LeaveAllocation = self.env['hr.leave.allocation'].sudo()
        for record in self:
            record.allocations_count = LeaveAllocation.search_count(
                [('leave_carryover_id', '=', record.id)]
            )
    ...
    def confirm_carryover(self):
        ...
        carryover_allocations = self.env['hr.leave.allocation'].search(
            [('leave_carryover_id', '=', self.id)])
        if len(carryover_allocations) > 0:
            carryover_allocations.action_approve()
            self.write({
                'state': 'done'
            })
    ...
    def _check_duplication(self):
        for record in self:
            if record.dest_leave_type_id == record.source_leave_type_id:
    ...
    def _check_scheduled_date(self):
        current_date = fields.Date.context_today(self)
        for record in self:
            if record.state == 'draft' and record.scheduled_date and record.scheduled_date <= current_date:
                raise ValidationError(_("Invalid Scheduled Date"))

    @api.constrains('carryover_percentage')
    def _check_carryover_percentage(self):
        for record in self:
            if record.carryover_percentage and record.state == 'draft':
                if record.carryover_percentage < 1 or record.carryover_percentage > 100:
                    raise ValidationError(_("Invalid Carry Over Percentage"))

    def unlink(self):
        if any(record.state != 'draft' for record in self):
            raise UserError(
                _('You can only delete carry over records that are in a draft state.')
            )
