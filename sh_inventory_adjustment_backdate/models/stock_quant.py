    ...
    def _apply_inventory(self, date=None):
        ...
        if backdate_quant:
            for quant in backdate_quant:
                super(StockQuant, quant.with_context(
                    sh_backdate=quant.sh_backdate,
                    sh_backdate_remark=quant.remarks_for_inventory_adj,
                ))._apply_inventory(date=quant.sh_backdate)
                quant.inventory_date = quant.sh_backdate
        remaining_quants = self - backdate_quant
        if remaining_quants:
            return super(StockQuant, remaining_quants)._apply_inventory(date=date)
        return True
